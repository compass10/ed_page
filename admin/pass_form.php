...
$pageTitle = $mode == 'write' ? '연도별합격자 작성' : '연도별합격자 수정';
include 'header.php';
...
if($mode == 'modify' && isset($_GET['bno']) && isset($board_table)) {
  ...
  if(!$row) {
    echo "<script>alert('데이터가 없습니다.'); location.href='pass_list.php';</script>";
    exit;
  }
}
?>
...
<div class="page_actions">
  <a href="pass_list.php" class="btn">← 목록으로</a>
</div>
...
<div class="form_section">
  <form method="post" action="content_ok.php" enctype="multipart/form-data">
    ...
    <input type="hidden" name="thumb_dir" value="pass">
    <?php if($mode == 'modify'): ?>
    ...
    <div class="form_group">
      ...
      <div style="margin-bottom: 10px;">
        <img src="<?=$_url?>thumb/pass/<?=$row['bimg']?>" style="max-width: 200px;">
      </div>
      <?php endif; ?>
    ...
    <div class="form_actions">
      ...
      <a href="pass_list.php" class="btn">취소</a>
      <?php if($mode == 'modify'): ?>
      <button type="button" class="btn danger" onclick="if(confirm('정말 삭제하시겠습니까?')) location.href='content_ok.php?mode=delete&bid=pass&bno=<?=$row['bno']?>'">삭제</button>
      <?php endif; ?>
    </div>
  </form>
</div>
