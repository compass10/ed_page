...
$pageTitle = $mode == 'write' ? $type_title . ' 갤러리 작성' : $type_title . ' 갤러리 수정';
include 'header.php';
...
if($mode == 'modify' && isset($_GET['bno']) && isset($board_table)) {
  ...
  if(!$row) {
    echo "<script>alert('데이터가 없습니다.'); location.href='gallery_list.php?type=$type';</script>";
    exit;
  }
}
?>
...
<div class="page_actions">
  <a href="gallery_list.php?type=<?=$type?>" class="btn">← <?=$type_title?> 목록으로</a>
</div>

<div class="form_section">
  <form method="post" action="gallery_ok.php" enctype="multipart/form-data">
    ...
    <div class="form_actions">
      ...
      <a href="gallery_list.php?type=<?=$type?>" class="btn">취소</a>
      <?php if($mode == 'modify'): ?>
      <button type="button" class="btn danger" onclick="if(confirm('정말 삭제하시겠습니까?')) location.href='gallery_ok.php?mode=delete&type=<?=$type?>&bno=<?=$row['bno']?>'">삭제</button>
      <?php endif; ?>
    </div>
  </form>
</div>
