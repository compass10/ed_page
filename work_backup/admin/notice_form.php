...
$pageTitle = $mode == 'write' ? '공지사항 작성' : '공지사항 수정';
include 'header.php';
...
if($mode == 'modify' && isset($_GET['bno']) && isset($board_table)) {
  ...
  if(!$row) {
    echo "<script>alert('데이터가 없습니다.'); location.href='notice_list.php';</script>";
    exit;
  }
}
?>
...
<div class="page_actions">
  <a href="notice_list.php" class="btn">← 목록으로</a>
</div>
...
<div class="form_section">
  <form method="post" action="board_ok.php" enctype="multipart/form-data">
    ...
    <div class="form_actions">
      ...
      <a href="notice_list.php" class="btn">취소</a>
      <?php if($mode == 'modify'): ?>
      <button type="button" class="btn danger" onclick="if(confirm('정말 삭제하시겠습니까?')) location.href='board_ok.php?mode=delete&bid=notice&bno=<?=$row['bno']?>'">삭제</button>
      <?php endif; ?>
    </div>
  </form>
</div>
