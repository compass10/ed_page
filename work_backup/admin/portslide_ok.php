...
$bcate = isset($_POST['bcate']) ? $_POST['bcate'] : '';
...
if($mode == 'delete') {
  ...
  echo "<script>alert('삭제되었습니다.'); location.href='portslide_list.php';</script>";
  exit;
}

// 등록
if($mode == 'write' || $mode == '') {
  $sql = "INSERT INTO $board_table (bid, btitle, bcate, is_hidden, bpw, bregdate)
    VALUES ('$bid', '".addslashes($btitle)."', '$bcate', '$is_hidden', '$bpw', NOW())";
  mysql_query($sql);
  $bno = mysql_insert_id();
...
if(isset($_FILES['bimg']) && $_FILES['bimg']['size'] > 0) {
  ...
  if(in_array($ext, array('jpg', 'jpeg', 'gif', 'png', 'webp'))) {
    $new_filename = 'portslide_' . $bno . '_' . time() . '.' . $ext;
...
echo "<script>alert('저장되었습니다.'); location.href='portslide_list.php';</script>";
@mysql_close($connect);
?>
